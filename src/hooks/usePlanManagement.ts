...
import { resolvePaceAnchor } from '../utils/paceAnchorResolver';
...
export function usePlanManagement() {
  ...
  const calculateTrainingPaces = (
    ...
    calibrationResult?: import('../types').CalibrationResult | null
  ): TrainingPaces | null => {
    const anchor = resolvePaceAnchor(calibrationResult, runnerAnswers);
    if (anchor.source.kind === 'default_estimated') return null;

    const paces: TrainingPaces = {
      ...anchor.paces,
      paceSourceLabel: anchor.source.label,
      paceConflictPct: anchor.conflictPct,
    };
    return paces;
  };
  ...
  const loadPlan = async (plan: TrainingPlan) => {
    ...
    if (normalizationResult.needsPersistence && plan.id) {
      logger.info('[LoadPlan] Plan needs persistence (migration-on-read) - writing to database', {
        planId: plan.id,
        userId: user?.id,
        wasConvertedFromWeekBased: normalizationResult.wasConvertedFromWeekBased,
        planStructureChanged: normalizationResult.planStructureChanged,
        needsPersistence: normalizationResult.needsPersistence,
        canonicalDaysCount: normalizationResult.canonicalDaysCount,
        derivedWeekSlotsCount: normalizationResult.derivedWeekSlotsCount,
        weeksCount: normalizationResult.weeksCount,
        invariantFailCount: normalizationResult.invariantFailures.length,
      });
    ...
    if (isDebugModeEnabled()) {
      setDebugInfo({
        ...
        normalizedWeeksCount: normalizationResult.weeksCount,
    ...
    const resolvedPaces = calculateTrainingPaces(plan.answers, plan.calibration_result);
    setTrainingPaces(resolvedPaces || plan.training_paces || null);
  ...
  const generatePreviewPlan = async (runnerAnswers: RunnerAnswers, readinessTier?: ReadinessTier): Promise<void> => {
    setAnswers(runnerAnswers);
    const paces = calculateTrainingPaces(runnerAnswers);
    setTrainingPaces(paces);
    setSavedPlanId(null);
    ...
    try {
      ...
      const response = await fetch(apiUrl, {
        ...
        body: JSON.stringify({
          ...
          startDate,
          trainingPaces: paces,
          readinessTier: readinessTier || 'green',
        }),
        signal: controller.signal,
      });
