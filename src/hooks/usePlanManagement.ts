...
export function usePlanManagement() {
  ...
  const loadPlan = async (plan: TrainingPlan) => {
    ...
    if (normalizationResult.wasNormalized && plan.id) {
      logger.info('[LoadPlan] Plan was normalized - persisting to database', {
        planId: plan.id,
        userId: user?.id,
      ...
      logger.info('[LoadPlan] Plan did not require normalization', {
        planId: plan.id,
        userId: user?.id
      });
    }
