...
import { ChatMessage, CalibrationResult } from '../types';
...
interface UseWorkoutOperationsProps {
  ...
  onCalibrationResultReady?: (result: CalibrationResult) => void;
...
export const useWorkoutOperations = ({
  ...
  const submitWorkoutCompletion = async (workoutRating: number) => {
    ...
    try {
      ...
      // Submit structured feedback to training_plan_workout_feedback table.
      // This powers the AI coach context — all completion data is stored here so
      // the chat function can reference real effort, HR, and notes data.
      try {
        const supabaseUrl = import.meta.env.VITE_SUPABASE_URL;
        const supabaseAnonKey = import.meta.env.VITE_SUPABASE_ANON_KEY;
        const { data: { session } } = await supabase.auth.getSession();
        if (supabaseUrl && supabaseAnonKey && session?.access_token) {
          fetch(`${supabaseUrl}/functions/v1/submit-workout-feedback`, {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
              training_plan_id: savedPlanId,
              workout_date: scheduledDate ?? new Date().toISOString().split('T')[0],
              week_number: workoutToRate.week,
              dow: workoutToRate.day,
              workout_text: workoutToRate.activity,
              rpe: workoutRating,
              completed: true,
              notes: workoutNotes.trim() || undefined,
            }),
          }).catch(err => logger.error('[WorkoutFeedback] submit-workout-feedback fire-and-forget error:', err));
        }
      } catch (feedbackSubmitError) {
        logger.error('[WorkoutFeedback] Error submitting structured feedback:', feedbackSubmitError);
      }
  ...
  const submitCalibrationCompletion = async () => {
    ...
    try {
      ...
      if (onCalibrationResultReady) {
        onCalibrationResultReady(calibrationResult as CalibrationResult);
      }
