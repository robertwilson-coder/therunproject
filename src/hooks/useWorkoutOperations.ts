...
export const useWorkoutOperations = ({
    ...
    try {
      if (isCompleted) {
        ...
        if (isCalibration) {
          ...
          setCalibrationAvgHeartRate(0);
          setCalibrationHeartRateDrift(0);
          ...
          setRating(0);
          setWorkoutDistance(parsed.distanceKm || 0);
  ...
  const submitWorkoutCompletion = async (workoutRating: number) => {
    ...
    try {
      ...
      logger.info('Workout completion saved successfully');
      ...
      if (planType === 'responsive' && onTriggerChat) {
        try {
          await checkForAIFeedback(user, savedPlanId, workoutToRate.week, workoutToRate.day, workoutToRate.activity, workoutRating, onTriggerChat);
        } catch (feedbackError) {
