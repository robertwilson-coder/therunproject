...
import { QuestionnairePartA } from './components/QuestionnairePartA';
import { ColourRevealScreen } from './components/ColourRevealScreen';
import { QuestionnairePartB } from './components/QuestionnairePartB';
...
import { calculateRaceFeasibilityAsync } from './utils/distanceFeasibilityEngine';
import type { FeasibilityResult, ReadinessTier } from './utils/distanceFeasibilityEngine';
...
type QuestionnaireStep = 'partA' | 'colourReveal' | 'partB';
...
function App() {
  ...
  const [pendingInterventionSignal, setPendingInterventionSignal] = useState<{ source: string; workoutKey: string; completionId?: string; ts: number } | null>(null);

  const [questionnaireStep, setQuestionnaireStep] = useState<QuestionnaireStep>('partA');
  const [partAAnswers, setPartAAnswers] = useState<Partial<RunnerAnswers>>({});
  const [feasibilityResult, setFeasibilityResult] = useState<{ result: FeasibilityResult; tier: ReadinessTier; weeksAvailable: number } | null>(null);
  ...
  const getRaceDistanceKm = (raceDistance: string): number => {
  ...
  const getWeeksToRace = (answers: Partial<RunnerAnswers>): number => {
    if (answers.raceDate && answers.customStartDate) {
      const start = new Date(answers.customStartDate);
      const race = new Date(answers.raceDate);
      return Math.max(1, Math.floor((race.getTime() - start.getTime()) / (1000 * 60 * 60 * 24 * 7)));
    }
    return answers.planWeeks || 12;
  };

  const resetQuestionnaireFlow = () => {
    setQuestionnaireStep('partA');
    setPartAAnswers({});
    setFeasibilityResult(null);
  };

  const handlePartASubmit = async (answers: Partial<RunnerAnswers>) => {
    const raceDistance = answers.raceDistance || 'Marathon';
    const raceDistanceKm = getRaceDistanceKm(raceDistance);
    const weeksToRace = getWeeksToRace(answers);

    try {
      const feasibility = await calculateRaceFeasibilityAsync({
        currentWeeklyVolume: answers.currentWeeklyKm || 0,
        currentLongestRun: answers.longestRun || 0,
        raceDistance: raceDistanceKm,
        weeksToRace,
      });

      setPartAAnswers(answers);
      setFeasibilityResult({ result: feasibility, tier: feasibility.readinessTier, weeksAvailable: weeksToRace });
      setQuestionnaireStep('colourReveal');
      window.scrollTo({ top: 0, behavior: 'instant' });
    } catch (err) {
      console.error('Feasibility calculation failed:', err);
      showToast('error', 'Failed to calculate feasibility. Please try again.');
    }
  };

  const handleColourContinue = () => {
    setQuestionnaireStep('partB');
    window.scrollTo({ top: 0, behavior: 'instant' });
  };

  const handlePartBSubmit = async (runnerAnswers: RunnerAnswers) => {
    const tier = feasibilityResult?.tier || 'green';
    try {
      await planManager.generatePreviewPlan(runnerAnswers, tier);
  ...
  const handleFeasibilityOption = async (runnerAnswers: RunnerAnswers) => {
    const raceDistance = runnerAnswers.raceDistance || 'Marathon';
    const raceDistanceKm = getRaceDistanceKm(raceDistance);
    const weeksToRace = getWeeksToRace(runnerAnswers);

    try {
      const feasibility = await calculateRaceFeasibilityAsync({
        currentWeeklyVolume: runnerAnswers.currentWeeklyKm || 0,
        currentLongestRun: runnerAnswers.longestRun || 0,
        raceDistance: raceDistanceKm,
        weeksToRace,
      });

      setPartAAnswers(runnerAnswers);
      setFeasibilityResult({ result: feasibility, tier: feasibility.readinessTier, weeksAvailable: weeksToRace });

      if (feasibility.readinessTier !== 'red') {
        setQuestionnaireStep('partB');
      } else {
        setQuestionnaireStep('colourReveal');
      }
  ...
  const handleAcceptPreview = async () => {
    try {
      ...
      await planManager.acceptPreviewPlan();
    ...
    <div className="min-h-dvh relative overflow-hidden">
      ...
      <div className="pt-16 pb-12 px-4 sm:px-6 lg:px-8">
                ...
                <div className="hidden md:flex items-center gap-2">
                  ...
                  {isAdmin && (
                    <button
                      ...
                      className="px-5 py-2.5 text-sm font-semibold transition-all duration-300 text-neutral-700 dark:text-neutral-300 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-xl hover:shadow-soft"
                    >
                      Admin
                    </button>
                  )}
                  ...
                  <div className="md:hidden fixed top-24 right-4 bg-white dark:bg-neutral-900 border-2 border-neutral-300 dark:border-neutral-700 rounded-2xl shadow-2xl z-[60] overflow-hidden animate-scale-in min-w-[200px]">
                    ...
                    {isAdmin && (
                      <button
                        ...
                        className="w-full px-6 py-4 text-left text-base font-bold text-neutral-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-all duration-300 border-b-2 border-neutral-200 dark:border-neutral-700 active:scale-95"
                      >
                        Admin
                      </button>
                    )}
          ...
          <div className="relative max-w-7xl mx-auto py-6 min-h-[700px] flex items-center overflow-hidden">
            ...
            <div className="relative z-10 w-full text-center px-4 pt-32 pb-16">
              ...
              <div className="flex flex-row gap-4 sm:gap-5 justify-center items-stretch px-4">
                <button
                  disabled
                  className="group relative flex-1 max-w-xs btn-primary text-base sm:text-lg py-3 sm:py-4 overflow-hidden whitespace-nowrap flex items-center justify-center opacity-50 cursor-not-allowed"
                >
                  <span className="relative z-10">
                    <span className="sm:hidden">Be Back Soon...</span>
                    <span className="hidden sm:inline">Be Back Soon...</span>
                  </span>
                </button>
                {user ? (
                  <button
        ...
        {navigation.appState === 'questionnaire' && (
          <div className="pt-8">
            {questionnaireStep === 'partA' && (
              <QuestionnairePartA
                onSubmit={handlePartASubmit}
                initialAnswers={partAAnswers}
              />
            )}
            {questionnaireStep === 'colourReveal' && feasibilityResult && (
              <ColourRevealScreen
                tier={feasibilityResult.tier}
                answers={partAAnswers}
                weeksAvailable={feasibilityResult.weeksAvailable}
                onContinue={handleColourContinue}
                onChangeTimeline={() => setQuestionnaireStep('partA')}
                onExtendTimeline={handleFeasibilityOption}
                onSwitchDistance={handleFeasibilityOption}
                onStartBaseBuild={handleFeasibilityOption}
              />
            )}
            {questionnaireStep === 'partB' && (
              <QuestionnairePartB
                partAAnswers={partAAnswers}
                onSubmit={handlePartBSubmit}
                onBack={() => setQuestionnaireStep('colourReveal')}
                isLoading={planManager.isLoading}
                tier={feasibilityResult?.tier ?? 'green'}
              />
            )}
          </div>
        )}
        ...
        {navigation.appState === 'viewPlan' && planManager.planData && planManager.planType && planManager.answers && (
          <Suspense fallback={<TrainingPlanSkeleton />}>
            <PlanWithChat
              ...
              onAnswersUpdate={planManager.setAnswers}
              pendingInterventionSignal={pendingInterventionSignal}
        ...
        {navigation.showRecoveryTools && (
          <Suspense fallback={<div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50"><div className="animate-spin rounded-full h-14 w-14 border-4 border-neutral-300 dark:border-neutral-800 border-t-primary-500"></div></div>}>
            <RecoveryTools
              ...
              savedPlanId={planManager.savedPlanId}
              onChatUpdate={(history) => planManager.setChatHistory(history)}
              currentChatHistory={planManager.chatHistory}
              onInterventionSent={(params) => {
                navigation.setShowRecoveryTools(false);
                navigation.setShowDashboard(false);
                navigation.setAppState('viewPlan');
                setPendingInterventionSignal({ ...params, ts: Date.now() });
              }}
            />
          </Suspense>
        )}
