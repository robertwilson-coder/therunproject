...
import { Users, TrendingUp, Activity, Calendar, RefreshCw, Bell, Mail } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const sendBulkNotification = async () => {
    if (!confirm('Send in-app notification to all users?')) return;
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/send-bulk-notification`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${session.access_token}`,
          ...
          body: JSON.stringify({
            title: 'Chat just got smarter',
            message: "Hey! I've been busy working on an improved coach chat — faster responses, better plan modifications, and smarter handling of schedule changes. Open the chat and give it a try.",
            email_text: "I've been busy working on the coach chat and it's now significantly better. Responses are faster, plan modifications are handled more intelligently, and rescheduling or load changes works more reliably. If you haven't used the chat in a while, now's a good time to try it.",
            type: 'info',
          }),
        }
      );
      const result = await response.json();
      if (result.success) {
        setBulkMessage(`Notification sent to ${result.count} users`);
      } else {
        setBulkMessage(`Failed: ${result.error}`);
      }
    } catch (error) {
  ...
  const sendBulkEmail = async () => {
    if (!confirm('Send bulk email to all users?')) return;
    ...
    try {
      ...
      if (!session) throw new Error('No session');
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/send-bulk-update-email`,
        {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${session.access_token}`,
      ...
      const result = await response.json();
      if (result.success) {
        setBulkMessage(`Email sent: ${result.results.sent} sent, ${result.results.failed} failed`);
      } else {
        setBulkMessage(`Failed: ${result.error}`);
      }
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            {bulkMessage && (
              <p className={`text-sm mt-2 ${bulkMessage.includes('Error') || bulkMessage.includes('Failed') ? 'text-red-500' : 'text-green-500'}`}>
                {bulkMessage}
              </p>
            )}
          </div>
