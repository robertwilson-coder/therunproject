...
export function WorkoutDayCard({
  ...
  const activityLower = activity.toLowerCase();
  const isCalibrationRun = !isRestDay && (workoutType === 'calibration' || activityLower.includes('calibration') ||
    (weekNumber === 1 &&
  ...
  const showCompletionButton = user && savedPlanId && !isPreviewMode && !isBeforeStart && !isRestDay;
    ...
    <div className="flex-1 flex flex-col relative">
      <div
        ...
        className={`rounded-lg ${isRestDay ? 'p-3' : 'p-4 md:p-5'} transition-all text-left w-full relative overflow-hidden ${
          ...
          !isBeforeStart && !isCalibrationRun ? 'hover:shadow-xl cursor-pointer active:scale-[0.99]' : 'cursor-default'
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${
                      isCompleted
                        ...
                        : 'bg-white/70 dark:bg-black/20 hover:bg-white dark:hover:bg-black/30 text-neutral-800 dark:text-neutral-100 border border-black/15 dark:border-white/15'
                    }`}
                  >
                    <Check className={`w-3.5 h-3.5 ${isCompleted ? 'stroke-[3]' : 'stroke-[2]'}`} />
