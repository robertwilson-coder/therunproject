      ...
      <div key={sectionIndex} className={sectionIndex > 0 ? 'mt-8' : ''}>
        {lines.map((line, lineIndex) => {
          if (line.startsWith('**') && line.endsWith(':**')) {
            return (
              <h3 key={lineIndex} className="text-xl font-black text-teal-300 mt-4 first:mt-0 uppercase tracking-wide">
          ...
          if (line.startsWith('• ')) {
            ...
            const formattedContent = content.replace(/\*\*(.*?)\*\*/g, '<strong class="text-white font-extrabold">$1</strong>');

            return (
              <div key={lineIndex} className="flex items-start gap-3 mb-3">
                <span className="text-teal-400 mt-1 font-bold">•</span>
                <p className="text-neutral-100 text-base leading-relaxed flex-1" dangerouslySetInnerHTML={{ __html: formattedContent }} />
              </div>
            );
          }
          ...
          const formattedLine = line.replace(/\*\*(.*?)\*\*/g, '<strong class="text-white font-extrabold">$1</strong>');
          return (
            <p key={lineIndex} className="text-neutral-100 text-base leading-relaxed mb-3" dangerouslySetInnerHTML={{ __html: formattedLine }} />
          );
        })}
      </div>
    );
...
export function WorkoutCelebrationModal({ celebrationMessage, onClose }: WorkoutCelebrationModalProps) {
  return (
    ...
    >
      <div
        className="card-premium w-full h-full max-w-full max-h-full rounded-none p-6 relative overflow-hidden animate-scale-in border-2 border-teal-400/50 shadow-2xl shadow-teal-500/30 flex flex-col"
      >
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          {[...Array(20)].map((_, i) => (
            <div
              key={i}
              className="absolute w-2 h-2 bg-teal-400 rounded-full animate-confetti"
              style={{
        ...
        <div className="relative z-10 flex flex-col h-full">
          <button
            onClick={onClose}
            className="absolute top-0 right-0 text-neutral-400 hover:text-white transition-colors z-20"
          >
            <X className="w-6 h-6" />
          </button>
          ...
          <div className="bg-neutral-950/90 rounded-xl p-6 md:p-8 border-2 border-teal-500/50 flex-1 overflow-y-auto shadow-inner mb-6">
