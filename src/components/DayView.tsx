...
import { dayOrder, convertRPEtoEffort, isDayBeforeStart, getWorkoutBgClass } from '../utils/trainingPlanUtils';
...
export function DayView({
  ...
  const outerBgClass = getWorkoutBgClass(activity, isCompleted);

  return (
    <div
      ref={containerRef}
      className={`rounded-2xl border border-neutral-200 dark:border-neutral-700 shadow-sm p-4 min-h-[70vh] flex flex-col transition-colors duration-300 ${outerBgClass}`}
      {...swipeHandlers}
    >
      ...
      <div className="flex items-center justify-between mb-6">
        <button
          ...
          disabled={!canGoBack}
          className={`p-2 rounded-lg transition-all bg-black/5 dark:bg-white/5 ${
            canGoBack
              ? 'text-neutral-900 dark:text-white hover:bg-black/10 dark:hover:bg-white/10'
