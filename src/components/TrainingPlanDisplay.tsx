...
export function TrainingPlanDisplay({
  ...
  useEffect(() => {
    if (currentWeek) {
      ...
      console.log('TrainingPlanDisplay - Mon workout:', currentWeek.days?.Mon?.workout);
        ...
        <div className="bg-white border-2 border-blue-500 rounded-lg p-4 md:p-6 mb-6 mx-4 shadow-lg">
          <div className="flex flex-col md:flex-row items-start gap-3 md:gap-4">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-base md:text-lg font-bold text-gray-900 mb-2">Preview Mode - Week {currentWeek.week} of {fullPlanData?.plan.length}</h3>
              <p className="text-sm md:text-base text-gray-700 mb-3 md:mb-4">
                {user
                  ? `You're viewing week ${currentWeek.week} of your training plan. Test the coach chat to make adjustments, then save your full ${fullPlanData?.plan.length}-week plan.`
                  : `You're viewing week ${currentWeek.week} of your training plan. Test the coach chat to make adjustments, then create an account to unlock and save your full ${fullPlanData?.plan.length}-week plan.`
                }
              </p>
              {onSaveFullPlan && (
                <button
                ...
                >
                  ...
                  {user ? 'Save & Unlock Full Plan' : 'Create Account & Unlock Full Plan'}
                </button>
              )}
            </div>
          </div>
