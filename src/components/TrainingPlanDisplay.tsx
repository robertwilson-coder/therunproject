import { Calendar, Clock, Zap, Activity, ChevronDown, BookOpen, Save, Check, List, TrendingUp, X, ChevronLeft, ChevronRight, ArrowRight, TrendingDown, Repeat, Undo, Heart, Target, Download } from 'lucide-react';
...
const extractRPEFromActivity = (activity: string): string | null => {
...
const getRPEDescription = (rpe: string, isBeginnerPlan: boolean): string => {
...
export function TrainingPlanDisplay({
  ...
  const getWorkoutCelebration = (activity: string): { title: string; message: string } => {
    const activityLower = activity.toLowerCase();
  ...
  const getPaceForWorkout = (activity: string): string | null => {
    ...
    // Check for structured workouts first (intervals, tempo, etc.)
    // Match patterns like: "5 x 1km", "5 x (1km", "6x400m", "8 x (2min"
    ...
    // Default to easy pace for easy/recovery runs and rest
  ...
  const loadUserTrainingPaces = async () => {
    ...
    try {
      ...
      console.error('Error loading training paces:', error);
    }
  };

  const getWorkoutHRZone = (activity: string) => {
  ...
  const toggleWorkoutCompletion = async (weekNumber: number, dayName: string, activity: string, e: React.MouseEvent) => {
    ...
    try {
      if (isCompleted) {
        ...
        setWorkoutDistance(parsed.distanceMiles ? parsed.distanceMiles * 1.60934 : 0);
        setWorkoutDuration(parsed.durationMinutes || 0);
      }
    } catch (error) {
  ...
  const submitWorkoutCompletion = async (workoutRating: number) => {
    ...
    try {
      // Convert km to miles for database storage (1 km = 0.621371 miles)
      const distanceInMiles = workoutDistance > 0 ? workoutDistance * 0.621371 : null;
      ...
      // Save workout notes if provided
      ...
      if (planType === 'responsive' && onTriggerChat) {
        await checkForAIFeedback(workoutToRate.week, workoutToRate.day, workoutToRate.activity, workoutRating);
      }
  ...
  const calculateWorkoutDate = (weekNumber: number, dayName: string): string => {
  ...
  const checkForAIFeedback = async (weekNumber: number, dayName: string, activity: string, rating: number) => {
    ...
    try {
      ...
      console.error('Error checking recent completions:', error);
    }
  ...
  const getTimeProgress = () => {
    if (!planStartDate || !answers?.raceDate) return null;
    ...
    const totalDays = Math.ceil((raceDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24));
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="px-4 md:px-0">
        <div className="mb-6 p-6 card-premium">
          <h2 className="text-2xl md:text-3xl font-bold text-neutral-900 dark:text-white">
            Your {answers?.raceDistance || 'Training'} Training Plan
          </h2>
          {!isPreviewMode && timeProgress ? (
            <div className="mt-4">
              ...
              <p className="text-xs text-neutral-500 dark:text-neutral-500 mt-2 text-center">
                {timeProgress.totalDays} days total • Race day: {new Date(answers.raceDate!).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
              </p>
            </div>
          ) : (
            <p className="text-neutral-700 dark:text-neutral-300 mt-2">
              {isPreviewMode ? 'Plan preview' : `${planData.plan.length} weeks of training`}
            </p>
          )}
        </div>
        ...
        {(viewMode === 'week' || isPreviewMode) && (
          <div className={`card-premium p-3 md:p-5 ${viewMode === 'calendar' ? 'mt-6' : ''}`}>
          ...
          <div className="space-y-3">
            {dayOrder.map((day) => {
              ...
              const extractedRPE = extractRPEFromActivity(activity);
              const rpeDescription = extractedRPE
                ? getRPEDescription(extractedRPE, isBeginnerPlan)
                : '';
              ...
              return (
                <div key={day} className="flex-1 flex flex-col" data-day-key={dayKey}>
                  <button
                    onClick={() => {
                      if (!isBeforeStart) {
                    ...
                    className={`rounded-lg ${isRestDay ? 'p-3' : 'p-4 md:p-5'} transition-all text-left w-full relative ${
                      isCompleted
                      ...
                      !isBeforeStart ? 'hover:shadow-lg hover:border-neutral-400 dark:hover:border-neutral-700 cursor-pointer active:scale-[0.99]' : 'cursor-default'
                    } border-2`}
                  >
                    ...
                    <div className="flex items-start justify-between gap-3 pr-6">
                      ...
                      <div className="flex-1">
                        <div
                          className={`text-sm md:text-base leading-relaxed font-medium mb-2 text-neutral-800 dark:text-neutral-100 ${
                            isCompleted ? 'line-through opacity-60' : ''
                          } ${isBeforeStart ? 'text-neutral-400 italic' : ''}`}
                          dangerouslySetInnerHTML={{ __html: renderMarkdown(activity) }}
                        />
                        {trainingPaces && !isRestDay && !activity.toLowerCase().includes('active recovery') && !activity.includes('/km') && !activity.includes('@ ') && (
                          <div className="text-xs text-primary-600 dark:text-primary-300 font-semibold mb-1">
                            Target Pace: {getPaceForWorkout(activity)}
                          </div>
                        )}
                        {!isRestDay && (() => {
                          const hrZone = getWorkoutHRZone(activity);
                          return hrZone && (
                  ...
                  {isExpanded && !isBeforeStart && (
                    <div className="mt-2 p-5 bg-neutral-100 dark:bg-neutral-800/90 border-2 border-primary-500/30 rounded-lg shadow-md">
                      ...
                      {!isRestDay && (() => {
                        const hrZone = getWorkoutHRZone(activity);
                        return hrZone && (
                          <div className="mb-4 p-4 bg-red-500/15 border-2 border-red-500/40 rounded-lg">
                            ...
                            <div className="text-sm text-neutral-800 dark:text-neutral-100 mb-2">
                              <span className="font-semibold">{hrZone.zone}:</span> {hrZone.range} <span className="text-neutral-600 dark:text-neutral-300">({hrZone.description})</span>
                            </div>
                            <div className="text-xs text-neutral-700 dark:text-neutral-200 italic bg-white/70 dark:bg-neutral-900/70 p-3 rounded border border-red-500/30">
                      ...
                      <ul className="space-y-3">
                        {coachingNotes.map((note, index) => (
                          <li key={index} className="text-sm text-neutral-800 dark:text-neutral-100 flex gap-3">
                            <span className="text-primary-600 dark:text-primary-300 font-bold text-lg leading-none">•</span>
                            <span className="leading-relaxed" dangerouslySetInnerHTML={{ __html: renderMarkdown(note) }} />
                          </li>
                        ))}
                      </ul>
                    </div>
                  ...
                  <div className="grid grid-cols-7 gap-1">
                    {dayOrder.map(targetDay => (
                      <button
                        ...
                        className={`p-2 text-xs font-medium rounded ${targetDay === pendingAction.data.dayName ? 'bg-neutral-800 text-neutral-600 cursor-not-allowed' : 'bg-primary-500/20 hover:bg-primary-500/30 text-primary-400 border border-primary-500/30'}`}
      ...
      {workoutToRate && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-start justify-center z-50 p-4 overflow-y-auto animate-fade-in">
          <div className="card-premium border-primary-500/30 max-w-md w-full p-6 relative z-50 my-8 animate-scale-in">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-bold text-white">Rate Your Workout</h3>
              <button
                onClick={() => {
                  setWorkoutToRate(null);
                  setRating(0);
            ...
            <div className="mb-6">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="relative pt-2 pb-2">
                    <input
                      ...
                      onChange={(e) => setWorkoutDistance(parseFloat(e.target.value))}
                  ...
                  <div className="text-center mt-2">
                    <span className="text-2xl font-bold text-primary-400">{workoutDistance.toFixed(1)} km</span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="text-center mt-2">
                    <span className="text-2xl font-bold text-primary-400">{workoutDuration} min</span>
                  </div>
                </div>
                ...
                <div className="mt-6 pt-6 border-t border-gray-200">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <button
                      type="button"
                      onClick={() => setWorkoutEnjoyment('terrible')}
                    ...
                    >
                      ...
                      <div className="text-xs font-medium text-neutral-300">Terrible</div>
                    </button>
                    <button
                      type="button"
                      onClick={() => setWorkoutEnjoyment('poor')}
                      className={`p-3 rounded-lg border-2 transition-all ${
                    ...
                    >
                      <div className="text-2xl mb-1">😞</div>
                      <div className="text-xs font-medium text-neutral-300">Poor</div>
                    </button>
                    <button
                      type="button"
                      onClick={() => setWorkoutEnjoyment('okay')}
                      className={`p-3 rounded-lg border-2 transition-all ${
                    ...
                    >
                      <div className="text-2xl mb-1">😐</div>
                      <div className="text-xs font-medium text-neutral-300">Okay</div>
                    </button>
                    <button
                      type="button"
                      onClick={() => setWorkoutEnjoyment('good')}
                      className={`p-3 rounded-lg border-2 transition-all ${
                    ...
                    >
                      <div className="text-2xl mb-1">🙂</div>
                      <div className="text-xs font-medium text-neutral-300">Good</div>
                    </button>
                    <button
                      type="button"
                      onClick={() => setWorkoutEnjoyment('great')}
                      className={`p-3 rounded-lg border-2 transition-all ${
                    ...
                    >
                      <div className="text-2xl mb-1">😊</div>
                      <div className="text-xs font-medium text-neutral-300">Great</div>
                    </button>
                    <button
                      type="button"
                      onClick={() => setWorkoutEnjoyment('amazing')}
                      className={`p-3 rounded-lg border-2 transition-all ${
                    ...
                    >
                      <div className="text-2xl mb-1">🤩</div>
                      <div className="text-xs font-medium text-neutral-300">Amazing</div>
                    </button>
                  </div>
                </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 px-4 py-2.5 border-2 border-neutral-700 text-neutral-300 rounded-lg hover:border-neutral-600 hover:bg-neutral-800 transition-all font-medium"
              >
                Cancel
              </button>
              <button
      ...
      {showCelebration && (
        ...
        >
          <div
            className="card-premium max-w-lg w-full p-8 relative overflow-hidden animate-scale-in"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Animated background gradient */}
            <div className="absolute inset-0 bg-gradient-to-br from-primary-500/20 via-green-500/20 to-blue-500/20 animate-pulse" />
            ...
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
              {[...Array(20)].map((_, i) => (
                <div
                  key={i}
                  className="absolute w-2 h-2 bg-primary-400 rounded-full animate-confetti"
                  style={{
            ...
            <div className="relative z-10">
              <button
                onClick={() => setShowCelebration(false)}
                className="absolute top-0 right-0 text-neutral-400 hover:text-white transition-colors"
              >
                <X className="w-6 h-6" />
              </button>

              <div className="text-center mb-6">
                <div className="w-20 h-20 mx-auto mb-4 bg-gradient-to-br from-primary-400 to-green-400 rounded-full flex items-center justify-center animate-bounce-slow">
              ...
              <div className="bg-neutral-900/50 rounded-lg p-6 border-2 border-primary-500/30">
