...
export function RecoveryTools({ onClose }: RecoveryToolsProps) {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .upsert({
          ...
          time_measured: hrForm.time_measured || null,
          notes: hrForm.notes
        }, {
          onConflict: 'user_id,log_date'
        });
      ...
      console.error('Error saving heart rate log:', error);
  ...
  const handleInjurySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error saving injury log:', error);
  ...
  const renderSleepTab = () => (
    <div className="space-y-6">
      <form onSubmit={handleSleepSubmit} className="space-y-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
  ...
  const renderHRTab = () => (
    <div className="space-y-6">
      <form onSubmit={handleHRSubmit} className="space-y-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Time Measured (optional)</label>
          <input
            type="time"
            value={hrForm.time_measured}
            onChange={(e) => setHrForm({ ...hrForm, time_measured: e.target.value })}
            className="w-full p-2 border-2 border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
          />
        </div>
  ...
  const renderInjuriesTab = () => (
    <div className="space-y-6">
      <form onSubmit={handleInjurySubmit} className="space-y-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
