...
import { checkForHRIntervention, checkForSleepIntervention, fetchRecentHRLogs, fetchRecentSleepLogs } from '../utils/recoveryFeedback';
...
export function RecoveryTools({ onClose, savedPlanId, onChatUpdate, currentChatHistory, onInterventionSent }: RecoveryToolsProps) {
  ...
  const handleSleepSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (savedPlanId) {
        const recentLogs = await fetchRecentSleepLogs(user.id, 7);
        await checkForSleepIntervention({
          user,
          savedPlanId,
          logDate: sleepForm.date,
  ...
  const handleHRSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (savedPlanId) {
        const recentLogs = await fetchRecentHRLogs(user.id, 8);
        await checkForHRIntervention({
          user,
          savedPlanId,
          logDate: hrForm.date,
          recentHRLogs: recentLogs,
          onChatUpdate,
          currentChatHistory: currentChatHistory || [],
          onInterventionSent
        });
