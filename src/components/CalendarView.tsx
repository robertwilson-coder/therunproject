...
export function CalendarView({ planData, completedWorkouts, planStartDate }: CalendarViewProps) {
  ...
  for (let i = 0; i < adjustedStartDay; i++) {
    blanks.push(<div key={`blank-${i}`} className="h-20 md:h-24 bg-gray-50 border border-gray-200"></div>);
  }
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    days.push(
      <div
        key={day}
        className={`h-20 md:h-24 border border-gray-200 p-1 md:p-2 overflow-hidden transition-all bg-white ${
          today ? 'ring-2 ring-blue-500' : ''
        } ${workout ? 'cursor-pointer hover:shadow-md hover:scale-105 hover:border-blue-400' : ''}`}
        onClick={() => workout && setSelectedWorkout(workout)}
      >
        <div className="flex items-start justify-between mb-1">
          <span className={`text-xs md:text-sm font-semibold ${today ? 'text-blue-600' : 'text-gray-700'}`}>
            {day}
          </span>
          {workout?.isCompleted && (
  ...
  return (
    <>
      <div className="bg-white border-2 border-gray-200 rounded-lg shadow-md p-4 md:p-6">
        <div className="flex flex-col gap-4 mb-6 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center justify-between sm:justify-start gap-3">
            ...
            <span className="text-base md:text-lg font-semibold text-gray-800 min-w-[140px] md:min-w-[180px] text-center">
        ...
        <div className="grid grid-cols-7 gap-0 border border-gray-200 rounded-lg overflow-hidden">
          {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day, idx) => (
            <div key={idx} className="bg-gray-100 text-gray-800 text-center py-2 md:py-3 font-bold text-xs md:text-sm border-r border-gray-200 last:border-r-0">
              {day}
            </div>
          ))}
          {blanks}
        ...
        <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-gray-700">Easy Run</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 bg-red-50 border-2 border-red-300 rounded"></div>
            <span className="text-gray-700">Intervals/Tempo</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 bg-orange-50 border-2 border-orange-300 rounded"></div>
            <span className="text-gray-700">Long Run</span>
          </div>
        </div>
      </div>
