...
export function CalendarView({ planData, completedWorkouts, planStartDate, onToggleCompletion }: CalendarViewProps) {
  const startDate = parseLocalDate(planStartDate);

  const [currentMonth, setCurrentMonth] = useState(() => {
    if (planData.days && planData.start_date) {
      return parseLocalDate(planData.start_date);
    }
    return new Date();
  });
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    days.push(
      <div
        ...
        } ${workout ? 'cursor-pointer hover:shadow-lg hover:scale-105 md:hover:scale-[1.02] active:scale-100 touch-manipulation' : ''}`}
