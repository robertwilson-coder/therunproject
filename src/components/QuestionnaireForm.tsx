...
export function QuestionnaireForm({ onSubmit, isLoading }: QuestionnaireFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-6">
      <div className="bg-neutral-900 border-2 border-neutral-800 rounded-xl p-6 shadow-lg">
        ...
        <div className="space-y-5">
                  ...
                  <button
                    ...
                    className={`px-4 py-3 rounded-lg border-2 font-medium transition-all hover:scale-105 ${
                      isSelected
                        ...
                        : 'bg-neutral-800 border-neutral-700 text-neutral-100 hover:border-primary-500'
                    }`}
                  >
                    {distance}
          ...
          <div className="space-y-4">
            <label className="flex items-center gap-3 cursor-pointer group">
              <input
                ...
                className="w-5 h-5 rounded border-2 border-neutral-700 bg-neutral-800 checked:bg-primary-500 checked:border-primary-500 focus:ring-0 focus:ring-offset-0 cursor-pointer transition-all"
              />
          ...
          <div>
            <label className="block text-sm font-medium text-white mb-2">
              ...
              {hasRaceDate && <span className="text-neutral-400 text-xs ml-2">(calculated from race date)</span>}
            </label>
            <input
              type="range"
              ...
              className={`w-full h-2 bg-neutral-800 rounded-lg appearance-none cursor-pointer accent-primary-500 ${hasRaceDate ? 'opacity-30 cursor-not-allowed' : ''}`}
              min="4"
              max="20"
              step="1"
            />
            <div className={`flex justify-between text-xs mt-1 ${hasRaceDate ? 'text-neutral-600' : 'text-neutral-400'}`}>
              <span>4 weeks</span>
              <span>20 weeks</span>
            </div>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-2 bg-neutral-800 text-white border-2 border-neutral-700 rounded-lg focus:ring-0 focus:border-primary-500 focus:outline-none transition-all placeholder:text-neutral-500"
              rows={3}
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="w-full mt-6 px-6 py-3 bg-primary-500 hover:bg-primary-600 text-white font-bold rounded-lg hover:scale-105 focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-95"
        >
      ...
      {showUltraDistanceModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
          <div className="bg-neutral-900 border-2 border-primary-500/30 rounded-xl max-w-md w-full p-6 animate-scale-in shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-bold text-white">Ultra Distance</h3>
              <button
            ...
            <div className="space-y-4">
              <p className="text-neutral-300 text-sm">
                Specify the distance of your ultra race in kilometers. Common distances include 50K, 100K, 50 miles (80K), 100 miles (161K), or any custom distance.
              </p>
      ...
      {isLoading && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
          <div className="bg-neutral-900 border-2 border-primary-500/30 rounded-xl max-w-md w-full p-8 animate-scale-in shadow-2xl">
