...
import { X, Plus, Trash2, Beaker, ClipboardList, TrendingUp, CreditCard as Edit2, CheckCircle, AlertCircle, Smile, Frown, Meh, ChevronDown, ChevronUp, Lightbulb } from 'lucide-react';
  ...
  const loadData = async () => {
    ...
    const [strategiesData, logsData, workoutsData] = await Promise.all([
      ...
      supabase
        ...
        .select('id, completed_at, day_name, distance_km, duration_minutes, rating')
        .eq('user_id', user.id)
                      ...
                      >
                        ...
                        {workouts.map((workout) => (
                          <option key={workout.id} value={workout.id}>
                            {new Date(workout.completed_at).toLocaleDateString()} - {workout.day_name} {workout.distance_km ? `(${workout.distance_km} km)` : ''}
                          </option>
                        ))}
                      </select>
                    </div>
