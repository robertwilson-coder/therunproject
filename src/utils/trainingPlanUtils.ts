...
export const renderMarkdown = (text: string) => {
  ...
  const hasStructuredFormat = /\b(warm up:|work:|cool down:)/i.test(processedText);
  if (hasStructuredFormat) {
    const summaryLinePattern = /^[^|\n]*?(run|session|tempo|interval)[^|\n]*?(\d+\s*(km|k|m|min))[^|\n]*?(at\s+\d+:\d+\s*\/km)?[^|\n]*?\n/i;
    processedText = processedText.replace(summaryLinePattern, '');
  }
  ...
  formatted = formatted
    ...
    .replace(/(<br>)(Warm up:)/gi, '$1<strong>$2</strong>')
    .replace(/(<br>)(Work:)/gi, '$1<strong>$2</strong>')
    .replace(/(<br>)(Cool down:)/gi, '$1<strong>$2</strong>')
    .replace(/^<br>/, '')
...
export const getWorkoutBgClass = (activity: string, isCompleted: boolean): string => {
  ...
  } else if (activityLower.includes('interval') || activityLower.includes('fartlek') || /\d+\s*x\s*[(\d]/.test(activityLower)) {
