...
export function parseWorkoutSections(description: string): WorkoutSections {
  ...
  const normalised = description.replace(/\s*\|\s*/g, ' | ');

  const warmUpRegex = /(?:\*\*)?(?:warm[\s-]?up|warmup)(?:\*\*)?[:\s]+(.+?)(?=\s*\|\s*(?:\*\*)?(?:work|main set|intervals|tempo|cool[\s-]?down|cooldown)|(?:\*\*)?(?:work|main set|intervals|tempo|cool[\s-]?down|cooldown)(?:\*\*)?[:\s]|$)/is;
  const workRegex = /(?:\*\*)?(?:work|main set|workout|intervals|tempo|repeats)(?:\*\*)?[:\s]+(.+?)(?=\s*\|\s*(?:\*\*)?(?:cool[\s-]?down|cooldown)|(?:\*\*)?(?:cool[\s-]?down|cooldown)(?:\*\*)?[:\s]|$)/is;
  const coolDownRegex = /(?:\*\*)?(?:cool[\s-]?down|cooldown)(?:\*\*)?[:\s]+(.+?)(?:\s*\|.*)?$/is;

  const warmUpMatch = normalised.match(warmUpRegex);
  if (warmUpMatch) {
    sections.warmUp = cleanSection(warmUpMatch[1]);
  }

  const workMatch = normalised.match(workRegex);
  if (workMatch) {
    sections.work = cleanSection(workMatch[1]);
  }

  const coolDownMatch = normalised.match(coolDownRegex);
  if (coolDownMatch) {
    sections.coolDown = cleanSection(coolDownMatch[1]);
  }
