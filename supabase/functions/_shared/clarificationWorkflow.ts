...
export interface PreviewOperation {
  operation: 'cancel' | 'reinstate' | 'move' | 'modify';
  workoutId: string;
  isoDate: string;
  displayDate: string;
...
export interface WorkoutModificationIntent {
  type: string;
  details: Record<string, unknown>;
}

export interface ModificationIntentResponse {
  mode: 'modification_intent';
...
export type ChatResponse = ClarificationRequest | CoachMessageOnly | PreviewMode | CommitResponse | ModificationIntentResponse;

export function createModificationIntentResponse(
  intent: ModificationIntent,
  modificationIntents: WorkoutModificationIntent[],
  message: string,
  reasoning: string
): ModificationIntentResponse {
  return {
    mode: 'modification_intent',
    intent,
    modificationIntents,
    message,
    reasoning,
    awaitingConfirmation: intent !== 'none' || modificationIntents.length > 0,
  };
}
